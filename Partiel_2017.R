...
nuagehxi <- function(x){ #m??thode matisse
  lastElem <- length(x)
...
displayFixedWidth <- function(x) {
  ...
  hist = hist(x, prob=T, breaks=seq(a0,ak,h), xlim=c(a0,ak), main="Histogramme ?? largeur fix??e")
  lines(hist$mids, hist$density, lwd=3, col="Red")
  lines(density(x), lwd=3, col="Blue")
  plot(ecdf(x), main="Fonction de r??partition empirique pour largeur fix??e")
}
...
displayFixedEffec <- function(x) {
  ...
  print(res)
  hist = hist(x, prob=T, breaks=res, main="Histogramme ?? effectif fix??")
  lines(hist$mids, hist$density, lwd=3, col="Red")
  lines(density(x), lwd=3, col="Blue")
  plot(ecdf(x), main="Fonction de r??partition empirique pour effectif fix??")
}
